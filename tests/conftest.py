import json
import os
import sys

import pytest

try:
    import invaultapi as api
except:
    sys.path.append(os.path.abspath(os.path.dirname(__file__) + "/.."))
    import invaultapi as api


@pytest.fixture
def testapi_url():
    return "http://10.0.152.131:8084/open/api/v1"

@pytest.fixture
def rsa_key():
    private_key = ""
    return api.RSAKey.import_key(private_key)

@pytest.fixture
def keyid():
    return "83c846b2433348da837b3aeb2e76fea6"

@pytest.fixture
def api_mock_data():
    apifile = os.path.dirname(__file__) + "/api.json"
    with open(apifile, "rb") as f:
        return json.load(f)
